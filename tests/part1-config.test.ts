...
describe("required fields reject when empty (exact §9 copy)", () => {
  const cases: { field: keyof Part1Input; message: string }[] = [
    ...
    { field: "year", message: "Enter a valid production year." },
    { field: "registrationNumber", message: "Enter a valid registration number." },
...
describe("optional fields persist null when empty, validate when present", () => {
  it("all optionals empty → null", () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      for (const f of ["price", "vin", "mileage", "color", "doorCount", "address", "notes"] as const) {
...
describe("field boundaries", () => {
  const reject: { name: string; input: Partial<Part1Input>; field: keyof Part1Input }[] = [
    ...
    { name: "year above current+1", input: { year: String(new Date().getFullYear() + 2) }, field: "year" },
    ...
    { name: "price above max", input: { price: "10000001" }, field: "price" },
    { name: "price negative", input: { price: "-5" }, field: "price" },
    { name: "doorCount 0", input: { doorCount: "0" }, field: "doorCount" },
  ...
  const accept: { name: string; input: Partial<Part1Input> }[] = [
    ...
    { name: "year current+1", input: { year: String(new Date().getFullYear() + 1) } },
    ...
    { name: "price max", input: { price: "10000000" } },
    { name: "price 2 decimals", input: { price: "0.99" } },
    { name: "doorCount 1", input: { doorCount: "1" } },
    { name: "doorCount 9", input: { doorCount: "9" } },
...
describe("isConfigUnlocked predicate", () => {
  ...
  it.each(["make", "model", "year", "registrationNumber", "fuelType", "transmission", "drive", "bodyType"] as const)(
