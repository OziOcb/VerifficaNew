...
| -------------------- | ------------------- | -------- | ------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| `price`              | Price               | No       | decimal | If present: numeric value `>= 0` and `<= 99999999.99` (matches the DB `numeric(10,2)` column), max 2 decimal places | trim, replace comma with dot, parse decimal                                     | `Enter a valid price greater than or equal to 0.`                   |
| `make`               | Make                | Yes      | text    | 1-50 characters after trim                                                                                          | trim, collapse repeated spaces, capitalize first letter                         | `Enter the car make.`                                               |
| `model`              | Model               | Yes      | text    | 1-60 characters after trim                                                                                          | trim, collapse repeated spaces, capitalize first letter                         | `Enter the car model.`                                              |
| `year`               | Year of production  | No       | integer | If present: 4 digits, `>= 1886`, `<= current year`                                                                  | trim, parse integer                                                             | `Enter a valid production year.`                                    |
| `registrationNumber` | Registration number | No       | text    | If present: 2-15 characters after trim; allowed chars: letters, digits, spaces, hyphen                              | trim, uppercase, collapse repeated spaces                                       | `Enter a valid registration number.`                                |
...
| `color`              | Color               | No       | text    | If present: 1-40 characters after trim                                                                              | trim, collapse repeated spaces, capitalize first letter                         | `Enter a valid color.`                                              |
...
| `doorCount`          | No of doors         | No       | integer | If present: whole number from 0 to 7                                                                                | trim, parse integer                                                             | `Enter a valid number of doors.`                                    |
| `address`            | Address             | No       | text    | If present: 5-150 characters after trim                                                                             | trim, collapse repeated spaces, capitalize first letter                         | `Enter a valid address.`                                            |
| `notes`              | Notes               | No       | text    | If present: up to 1000 characters                                                                                   | preserve line breaks, trim leading/trailing whitespace, capitalize first letter | `Notes cannot be longer than 1000 characters.`                      |

> **Required set (gates Parts 2-5, per PRD FR-013):** `make`, `model`, `fuelType`,
> `transmission`, `drive`, `bodyType`. `year` and `registrationNumber` are
> **optional** — they feed the dashboard tile title only (FR-006), not the
> question-visibility logic, so they must not block the Part 1 unlock.
