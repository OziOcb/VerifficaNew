...
- [ ] 1.2 Unit/integration suite passes (`npm test`)
- [ ] 1.3 New drain suite fails when `orderBy("seq")` is reverted (FIFO-tie case red)

#### Manual

- [ ] 1.4 Narrowed Stryker run over `src/lib/sync.ts:159-192`; survivors reviewed one-by-one; assertions added only for real durability bugs; accepted survivors recorded
...
- [ ] 3.1 Lint/type pass (`npm run lint`)
- [ ] 3.2 `npm run test:e2e` green end-to-end (setup → seed → offline-durability → teardown)
- [ ] 3.3 Spec fails when reconnect/drain is skipped (per-write SSR assertions go red)

#### Manual

- [ ] 3.4 Headed run: offline reload serves the SW shell (not Chrome's offline page); notes/config persist visually
- [ ] 3.5 No orphaned inspection rows remain for the shared user after the run
- [ ] 3.6 `context/foundation/test-plan.md` updated: §6.3 multi-write pattern, §3 Phase 2 status, §6.6 per-phase note
