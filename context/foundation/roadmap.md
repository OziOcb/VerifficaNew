...
- **Outcome:** (foundation) On-device store, Change Queue, and background Last-Write-Wins sync landed as the local-first persistence contract; one domain record survives a full offline → online cycle with no loss.
...
- **Status:** in progress (F-01 prerequisite met; change folder `offline-first-persistence-layer` + branch `feat/offline-first-persistence-layer` created; unknowns/risk resolved via `research.md` — planning next)
