...
## Per-section answer polarity is interpretation, not severity weighting — keep it, but never add per-question weights or a single verdict

- **Context**: The S-06 scoring/summary surfaces on veriffica — anything computing the
  Yes/No/Don't-know aggregation for the Total Score, per-Part charts, or answer coloring
  (`src/lib/answers.ts`, `DistributionBar`, `SummaryScreen`, `SessionScreen`). Applies
  whenever FR-019 ("no single quality score, no severity weighting") is weighed against the
  idea doc's directional answer meaning (`idea/veriffica-instruction.md:25-27`).
- **Problem**: The idea doc gives "Yes" a directional meaning (Section 1/Parts 2–4 Yes →
  worse car; Section 2/Part 5 Yes → more reliable), which conflicts on its face with FR-019's
  no-weighting/no-verdict rule. Two opposite failure modes follow: (a) a reviewer wrongly
  flags the per-section sentiment coloring as an FR-019 violation and strips it, making a
  clean car (all "No" in Parts 2–4) render alarmingly red; or (b) someone "restores accuracy"
  by reintroducing per-question severity weights (broken turbo > broken mirror) or a single
  collapsed buy/don't-buy score — which IS the FR-019 non-goal.
- **Rule**: Keep per-section answer POLARITY as interpretation only — Parts 2–4: No =
  positive; Part 5: Yes = positive; `dont_know` = unknown — every question still counts
  equally (`positiveAnswer(part)` + `sentimentDistribution()` in `@/lib/answers`). Never add
  per-question severity weights, and never collapse the distribution into a single quality
